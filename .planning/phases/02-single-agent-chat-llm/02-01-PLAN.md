...
must_haves:
  ...
  artifacts:
    ...
    - path: "crates/boternity-types/src/chat.rs"
      provides: "ChatSession, ChatMessage, SessionStatus, MessageRole"
  ...
  key_links:
    ...
    - from: "crates/boternity-core/src/chat/repository.rs"
      ...
      via: "imports ChatSession, ChatMessage"
...
**chat.rs** -- Chat session and message types:
...
Run `cargo check -p boternity-types` -- all new types compile without errors.
  ...
  <done>All Phase 2 domain types exist in boternity-types with proper derives (Serialize, Deserialize, Debug, Clone), Display/FromStr for enums, and UUIDv7 ID patterns matching existing bot.rs conventions.</done>
</task>
...
- Also add `fn save_context_summary(&self, ...)` and `fn get_latest_summary(&self, session_id: &Uuid)` for the sliding window context summaries.

Define a `ContextSummary` struct either in boternity-types/src/memory.rs or boternity-types/src/chat.rs: id (Uuid), session_id (Uuid), summary (String), messages_start (u32), messages_end (u32), token_count (u32), created_at (DateTime<Utc>).
...
Run `cargo check -p boternity-core` -- all traits compile. Run `cargo check --workspace` to verify no dependency violations (core must not depend on infra).
  ...
  <done>ChatRepository and MemoryRepository traits exist in boternity-core with full CRUD methods using RPITIT pattern. ContextSummary type exists for sliding window management. The workspace compiles cleanly with zero core->infra dependencies.</done>
</task>
