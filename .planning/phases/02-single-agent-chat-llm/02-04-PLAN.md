...
- Private `ContextSummaryRow` struct for mapping context_summaries table rows
...
Import `ContextSummary` from `boternity_types::chat::ContextSummary` (the definitive location established in Plan 01 Task 1).
...
Run `cargo check -p boternity-infra` -- both repositories compile and implement their respective traits. Verify that writer pool is used for mutations and reader pool for queries.
  ...
  <done>SqliteChatRepository implements ChatRepository (including save_context_summary and get_latest_summary) and SqliteMemoryRepository implements MemoryRepository, both with raw sqlx queries, split read/write pools, and private Row mapping structs. Migration creates all 5 tables.</done>
</task>
...
3. SqliteChatRepository implements ChatRepository (including context summary methods)
...
8. ContextSummary imported from boternity_types::chat (not ambiguous)
</verification>

<success_criteria>
- Migration creates chat_sessions, chat_messages, session_memories, pending_memory_extractions, context_summaries tables
- SqliteChatRepository provides full session and message CRUD plus context summary persistence
- SqliteMemoryRepository provides memory storage, retrieval, and pending extraction queue
