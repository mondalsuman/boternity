...
must_haves:
  truths:
    ...
    - "Conversation survives force-quit (Ctrl+C) without message loss"
...
<task type="auto">
  <name>Task 2: Create chat loop orchestration in loop_runner.rs</name>
  <files>
    crates/boternity-api/src/cli/chat/loop_runner.rs
  </files>
  <action>
Create the file `loop_runner.rs` (NOT `loop.rs` -- avoids Rust `loop` keyword conflict).
...
8. **Chat loop:**
   ...
   - Loop:
     ...
     e. Save user message to DB immediately (ensures crash resilience)
...
- On other errors: log and continue (don't crash the chat loop)
  ...
  <verify>
Run `cargo check -p boternity-api` -- loop_runner.rs compiles. Verify it imports from agent engine, chat service, renderer, input, banner, and commands modules.
  </verify>
  <done>The chat loop in loop_runner.rs orchestrates bot resolution, LLM provider creation, agent engine execution, streaming display, session persistence, memory extraction, and title generation. Messages are persisted immediately after each exchange for crash resilience.</done>
</task>

<task type="auto">
  <name>Task 3: Wire chat module into AppState, CLI commands, and main.rs</name>
  <files>
    crates/boternity-api/src/state.rs
    crates/boternity-api/src/main.rs
    crates/boternity-api/src/cli/mod.rs
...
Initialize tracing: In main.rs, replace the current tracing_subscriber setup with `boternity_observe::tracing_setup::init_tracing(false)` (OTel disabled by default, can be enabled with --otel flag later).
  ...
  <verify>
Run `cargo build --release -p boternity-api` to verify the binary builds. Test basic command parsing: `cargo run -p boternity-api -- chat --help` should show the chat subcommand with slug argument and --resume option.
  </verify>
  <done>`bnity chat <bot>` is wired as a CLI command. AppState includes chat_service and memory repositories. The Commands enum routes Chat to loop_runner::run_chat_loop.</done>
</task>
...
- Conversation survives force-quit without message loss (messages saved immediately)
- Memory extraction runs periodically and at session end
