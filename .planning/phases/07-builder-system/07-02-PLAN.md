...
must_haves:
  truths:
    ...
    - "Builder memory recall is injected into the Forge system prompt"
  artifacts:
...
<task type="auto">
  <name>Task 2: Forge system prompt builder with memory recall</name>
  <files>
    crates/boternity-core/src/builder/prompt.rs
  </files>
  <action>
    ...
    **`pub fn build_forge_system_prompt(state: &BuilderState, mode: BuilderMode, recalled_memories: &[RecalledBuilderMemory]) -> String`**

    The third parameter `recalled_memories` accepts past session data from the BuilderMemoryStore. This is how Forge implements the user decision: "remembers past builder sessions and suggests similar choices ('Last time you made a coding bot, you chose formal tone -- same here?')".

    **RecalledBuilderMemory** struct (define in this file or in builder types):
    `initial_description: String, chosen_tone: Option<String>, chosen_model: Option<String>, chosen_skills: Vec<String>, bot_slug: Option<String>`.
    This is a lightweight view of BuilderMemoryEntry for prompt injection -- the caller (LlmBuilderAgent in Plan 07-05) queries the memory store and passes results here.
    ...
    {if NewSkill: "Ask about skill name, description, type (local/wasm), trigger conditions, and required capabilities. When done, signal ready_to_assemble with the skill config."}
    </builder_instructions>
    ...
    {if recalled_memories is non-empty:}
    <past_sessions>
    The user has created bots before. Here are relevant past choices you can reference to suggest similar options:
    {for each memory in recalled_memories:}
    - Previously created "{memory.bot_slug or 'a bot'}": description "{memory.initial_description}", tone: {memory.chosen_tone or "not set"}, model: {memory.chosen_model or "default"}, skills: [{memory.chosen_skills.join(", ")}]
    {end for}
    When relevant, mention these past choices: "Last time you made a similar bot, you chose X -- would you like the same here?"
    </past_sessions>
    ```

    If `recalled_memories` is empty, omit the `<past_sessions>` section entirely (no empty XML tags).
  ...
  <verify>
    ...
    - test build_forge_system_prompt with non-empty recalled_memories includes `<past_sessions>` section
    - test build_forge_system_prompt with empty recalled_memories omits `<past_sessions>` section
  ...
  <done>Forge system prompt builder produces XML-tagged prompt with accumulated state context and builder memory recall. Past session suggestions injected via recalled_memories parameter. Forge identity embedded as const. Phase labels defined.</done>
</task>
...
- Builder memory recall injected into system prompt via recalled_memories parameter
</verification>
...
- Forge system prompt includes past_sessions section when recalled_memories is non-empty
- RecalledBuilderMemory type defined for prompt injection
